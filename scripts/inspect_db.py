...
async def inspect():
    async with engine.connect() as conn:
        ...
        try:
            res = await conn.execute(text("SELECT slug, name, allow_split FROM permission_policies"))
            rows = res.fetchall()
            print(f"Policy Count: {len(rows)}")
            for r in rows:
                print(f"- {r.slug}: {r.name} (Split: {r.allow_split})")
